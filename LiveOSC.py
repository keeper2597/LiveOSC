...
# Standard Ableton Methods
    ...
    def current_song_time_changed(self):
        ...
        if int(time) != self.time:
            self.time = int(time)
            self.oscEndpoint.send("/beat", self.time)
...
# Add / Remove Listeners   
    ...
    def overdub_change(self):
        ...
        self.oscEndpoint.send("/overdub", (int(overdub) + 1))
...
# Listener Callbacks
    ...
    def clip_position(self, clip, tid, cid):
        #if self.check_md(1):
            if clip.is_playing:
                self.oscEndpoint.send('/clip/position', (tid, cid, clip.playing_position, clip.length, clip.loop_start, clip.loop_end))
    ...
    def slot_changestate(self, slot, tid, cid):
        ...
        if slot.clip != None:
            ...
            self.oscEndpoint.send('/track/info', (tid, armed, cid, playing, length))
            self.oscEndpoint.send('/name/clip', (tid, cid, str(slot.clip.name), slot.clip.color))
        else:
    ...
    def clip_changestate(self, clip, x, y):
        ...
        self.oscEndpoint.send('/clip/info', (x, y, playing))
    ...
    def mixerv_changestate(self, type, tid, track, r = 0):
        ...
        if r == 2:
            self.oscEndpoint.send('/master/' + types[type], (float(val)))
        elif r == 1:
            self.oscEndpoint.send('/return/' + types[type], (tid, float(val)))
        else:
            self.oscEndpoint.send('/' + types[type], (tid, float(val)))        
        
    def mixert_changestate(self, type, tid, track, r = 0):
        val = eval("track." + type)
        
        if r == 1:
            self.oscEndpoint.send('/return/' + type, (tid, int(val)))
        else:
            self.oscEndpoint.send('/' + type, (tid, int(val)))        
    
    def send_changestate(self, tid, track, sid, send, r = 0):
        val = send.value
        
        if r == 1:
            self.oscEndpoint.send('/return/send', (tid, sid, float(val)))   
        else:
            self.oscEndpoint.send('/send', (tid, sid, float(val)))   
    ...
    def meter_changestate(self, tid, track, lr, r = 0):
        if r == 2:
            if self.check_md(2):
                if lr == 0:
                    self.oscEndpoint.send('/master/meter', (0, float(track.output_meter_left)))
                else:
                    self.oscEndpoint.send('/master/meter', (1, float(track.output_meter_right)))
        elif r == 1:
            if self.check_md(3):
                if lr == 0:
                    self.oscEndpoint.send('/return/meter', (tid, 0, float(track.output_meter_left)))
                else:
                    self.oscEndpoint.send('/return/meter', (tid, 1, float(track.output_meter_right)))        
        else:
            if self.check_md(4) and False:
                if lr == 0:
                    self.oscEndpoint.send('/track/meter', (tid, 0, float(track.output_meter_left)))
                else:
                    self.oscEndpoint.send('/track/meter', (tid, 1, float(track.output_meter_right)))
    ...
    def param_changestate(self, param, tid, did, pid, type):
        if type == 2:
            self.oscEndpoint.send('/master/device/param', (did, pid, param.value, str(param.name)))
        elif type == 1:
            self.oscEndpoint.send('/return/device/param', (tid, did, pid, param.value, str(param.name)))
        else:
            self.oscEndpoint.send('/device/param', (tid, did, pid, param.value, str(param.name)))
    ...
    def device_changestate(self, track, tid, type):
        ...
        if type == 2:
            self.oscEndpoint.send('/master/devices/selected', (did))
        elif type == 1:
            self.oscEndpoint.send('/return/device/selected', (tid, did))
        else:
            self.oscEndpoint.send('/device/selected', (tid, did))        
